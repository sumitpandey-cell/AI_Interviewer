...
const InterviewHistoryPanel = ({ 
  ...
  // If no interviews, show placeholder
    ...
    return (
      <Card className="bg-card-bg border-primary/20">
        <CardHeader className="pb-3">
          ...
          <CardDescription>No interviews yet. Start your first interview from the card above.</CardDescription>
        </CardHeader>
      </Card>
    );
  }
  ...
  const processedInterviews = interviews.map(interview => ({
    ...
    score: interview.status === 'completed' ? getRandomScore() : undefined,
